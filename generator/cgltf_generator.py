...
PREFIX = """# Yet another cgltf wrapper for Ruby
#
...
def sanitize_macro(ctx):
    ...
    # refer mapping
    for macro_name, macro_value in ctx.decl_macros.items():
        define_mapping = cgltf_parser.get_define_mapping(macro_name)
        if define_mapping:
...
def sanitize_struct(ctx):
    for struct_name, struct_info in ctx.decl_structs.items():
        ...
        for field in struct_info.fields:
            field.type_kind = cgltf_parser.get_cindex_ctypes_mapping(str(field.type_kind), field.type_name)
    ...
    for typedef_name, typedef_info in ctx.decl_typedefs.items():
        if typedef_info.func_proto != None:
            for arg in typedef_info.func_proto.args:
                arg.type_kind = cgltf_parser.get_cindex_ctypes_mapping(str(arg.type_kind), arg.type_name)
            typedef_info.func_proto.retval.type_kind = cgltf_parser.get_cindex_ctypes_mapping(str(typedef_info.func_proto.retval.type_kind), typedef_info.func_proto.retval.type_name)
        else:
            if str(typedef_info.type_kind) == "TypeKind.RECORD":
                typedef_info.type_kind = None
            else:
                typedef_info.type_kind = cgltf_parser.get_cindex_ctypes_mapping(str(typedef_info.type_kind), typedef_info.name)
...
def sanitize_function(ctx):
    for func_name, func_info in ctx.decl_functions.items():
        ...
        for arg in func_info.args:
            if cgltf_parser.is_cgltf_callback_type(arg.type_kind, arg.type_name):
                arg.type_kind = ":" + arg.type_name
            else:
                arg.type_kind = cgltf_parser.get_cindex_ctypes_mapping(str(arg.type_kind), arg.type_name)
        func_info.retval.type_kind = cgltf_parser.get_cindex_ctypes_mapping(str(func_info.retval.type_kind), func_info.retval.type_name)
...
def generate_enum(ctx, indent = ""):
    for enum_name, enum_value in ctx.decl_enums.items():
        for enum in enum_value:
            print(indent + "%s = %s" % (enum[0].capitalize(), enum[1]), file = sys.stdout)
    ...
    for struct_name, struct_info in ctx.decl_structs.items():
        ...
        for field in struct_info.fields:
            if field.element_count <= 1:
                ...
                member_is_record = cgltf_parser.query_cgltf_cindex_mapping_entry_exists(field.type_kind) and cgltf_parser.get_cgltf_cindex_mapping_value(field.type_kind) == "TypeKind.RECORD"
                type_kind = field.type_kind.capitalize() + '.by_value' if member_is_record else field.type_kind
                print(indent + "    :%s, %s," % (field.element_name, type_kind), file = sys.stdout)
            else:
...
def generate_function(ctx, indent = "", module_name = ""):
    ...
    for func_name, func_info in ctx.decl_functions.items():
        ...
        if len(func_info.args) > 0:
            ...
            # Capitalize and add ".by_value" to struct arguments (e.g.: Color -> Color.by_value)
            arg_is_record = lambda arg: cgltf_parser.query_cgltf_cindex_mapping_entry_exists(arg) and cgltf_parser.get_cgltf_cindex_mapping_value(arg) == "TypeKind.RECORD"
    ...
    for func_name, func_info in ctx.decl_functions.items():
        ...
        retval_is_record = cgltf_parser.query_cgltf_cindex_mapping_entry_exists(func_info.retval.type_kind) and cgltf_parser.get_cgltf_cindex_mapping_value(func_info.retval.type_kind) == "TypeKind.RECORD"
...
def generate(ctx, prefix = PREFIX, postfix = POSTFIX, *, module_name = "", table_prefix = "Cgltf_", typedef_prefix="", typedef_postfix="", struct_prefix="", struct_postfix="", struct_alias=None):
