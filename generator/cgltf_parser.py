...
def generate_type_mapping(headers_list_filename = './cgltf_headers_list.json'):
    ...
    ### cgltf_size (== size_t) -> ULONG conversion
    for typedef_name, typedef_info in ctx.decl_typedefs.items():
        if typedef_name == 'cgltf_size':
            ctx.decl_typedefs['cgltf_size'].type_kind = TypeKind.ULONG
...
def generate_define_list(headers_list_filename = './cgltf_headers_list.json', concatinate = True):
    headers_list_file = Path(headers_list_filename)
...
def _init_define_mapping(mapping_filename = './cgltf_define_mapping.json'):
...
def _init_type_mapping(mapping_filename = './cgltf_cindex_mapping.json'):
    mapping_file = Path(mapping_filename)
...
def register_cgltf_cindex_mapping(strTypeKind, strCgltfTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    if strCgltfTypedef not in cindex_mapping:
        cindex_mapping[strCgltfTypedef] = strTypeKind

def query_cgltf_cindex_mapping_entry_exists(strCgltfTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    return strCgltfTypedef in cindex_mapping

def get_cgltf_cindex_mapping_value(strCgltfTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    return cindex_mapping[strCgltfTypedef]


def get_cindex_ctypes_mapping(strTypeKind, strCgltfTypedef):
    # Remove const declaration
    strCgltfTypedef = re.sub(r"^\s*const\s*", '', strCgltfTypedef)

    if strTypeKind == 'TypeKind.RECORD':
        return strCgltfTypedef

    # Handle variable length arguments
    # - Check special marks recoreded in collect_decl_function
    if strTypeKind == 'TypeKind.INVALID' and strCgltfTypedef == 'va_list':
        return ':varargs'

    isPointerToChar = False
    if strTypeKind == 'TypeKind.POINTER':
        pattern = re.compile(r"\s*char\s*\*")
        m = re.search(pattern, strCgltfTypedef)
        if m:
    ...
    if isPointerToChar:
        ...
        if strTypeKind != 'TypeKind.TYPEDEF':
            ...
            mapping_key = get_cgltf_cindex_mapping_value(strCgltfTypedef)

        if mapping_key == "TypeKind.RECORD":
            return str(strCgltfTypedef)
        else:
            return ctypes_mapping[mapping_key]

def is_cgltf_callback_type(strTypeKind, strCgltfTypedef):
    return query_cgltf_cindex_mapping_entry_exists(strCgltfTypedef) and (get_cgltf_cindex_mapping_value(strCgltfTypedef) == "TypeKind.FUNCTIONPROTO")
...
def collect_decl_struct(ctx, cursor, struct_name=None, typedef_name=None):
    ...
    for field in cursor.get_children():
        ...
        field_info.type_kind = field.type.kind # canonical_kind
...
def execute(ctx, arg=[]):
    ...
    try:
        ...
        for typedef_name, typedef_info in ctx.decl_typedefs.items():
            register_cgltf_cindex_mapping(str(typedef_info.type_kind), typedef_name)
